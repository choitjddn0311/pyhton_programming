...
#글꼴 설정
font = pygame.font.Font('resources/Katuri.ttf', 30) #글꼴 파일명, 크기
font2 = pygame.font.SysFont('맑은고딕', 30) #시스템 글꼴명, 크기
white = (255,255,255)
red = (255,0,0)

def write(string, x, y, color, size = 20):
    global pad
    font = pygame.font.Font('resources/Katuri.ttf', size) #글꼴 설정
    text = font.render(string, False, color)
    pad.blit(text, (x,y)) #텍스트를 그림
...
rx = random.randint(0, width - rw) # 운석의 x 좌표
ry = 0 
rs = 2 # 운석 속도
...
while True:
    ...
    if len(mlist) != 0: #미사일 리스트가 비어있지 않다면
        for mis in mlist:
            mis[1] -= 10 #미사일이 전진함
